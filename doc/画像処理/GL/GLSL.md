...
# ビルトイン関数
## texture2D
- varying 変数 vary_uv として、頂点シェーダから UV 座標を受け取る。varying の「頂点間で値が補完される」という特性は、UV 座標に関しても同様。
- なので、ピクセルごとの正確な UV 座標をフラグメントシェーダが把握できるようになる。

- ピクセル単位の UV 座標が分かったら、その UV 座標を元にテクスチャからテクセル情報＝色を取り出す。
